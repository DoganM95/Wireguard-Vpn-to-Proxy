...
ENTRYPOINT ["sh", "-c", "\
    ...
    ip route add default via $(ip route show | awk '/default/ {print $3}' | head -n1) dev eth0 && \
    echo 'nameserver 8.8.8.8' > /etc/resolv.conf && \
    API_IP=$(getent ahostsv4 api.ipify.org | head -n1 | awk '{print $1}') && \
    ip route add ${API_IP}/32 dev wg0 && \
    ip route add 10.0.0.0/24 via $(ip route show | awk '/default/ {print $3}' | head -n1) dev eth0 && \
